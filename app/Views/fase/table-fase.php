                ...
                <div class="card-body table-responsive p-0" style="height: 300px;">
                    <table class="table table-head-fixed text-nowrap">
                        ...
                        <?php foreach ($fases as $f) : ?>
                            <tr>
                                <td style="visibility: hidden;"><?= $f['id_fase'] ?></td>
                                <td><?= $f['nombre'] ?></td>
                                <td><?= $f['fecha_inicio'] ?></td>
                                <td><?= $f['fecha_fin'] ?></td>
                                <td>
                                    <a href="<?php echo base_url('/fase/delete/'.$f['id_fase']); ?>" onclick= "return bajaFase()"><i class="fa-solid fa-trash-can"></i></a>

                                    <a href="#" onclick="actualizarFase(<?= $f['id_fase']?>, '<?php echo $f['nombre'] ?>','<?php echo $f['fecha_inicio'] ?>', '<?php echo $f['fecha_fin'] ?>')">
                                        <i class="fa-solid fa-pen"></i>
                                    </a>
                                </td>
                                <td>
                                    <a href="<?php echo base_url('/list-partido/'.$f['id_fase']); ?>">Agregar partido</a>
                                </td>
                                <td>
                                    <a href="<?php echo base_url('/get-partidos/'.$f['id_fase']); ?>">Ver partidos</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                        ...
                        <div class="modal-body">
                            <form action="<?= site_url('/submit-fase')?>" method="POST" id="formulario" style="width: 100%">
                                <div class="form-row col-md-12">
                                    <input type="hidden" class="form-control" name="id_fase" id="id_fase">
